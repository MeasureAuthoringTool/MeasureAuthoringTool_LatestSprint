...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:key name="NON_DC_IDS" match="*[@id and (not(ancestor::elementlookup) or ancestor::measurecalc) and not(ancestor::abbreviations)]" use="@id" />
   <xsl:key name="NON_DC_TOS" match="*[@to and not(ancestor::elementlookup) and not(ancestor::abbreviations)]" use="@to" />
   <xsl:key name="IQDSEL_IDS" match="iqdsel[@refid]" use="@refid" />
   ...
   <xsl:template name="population_criteria">
      ...
      <component typeCode="COMP">
         <section>
            ...
            <text/>
            <xsl:for-each select="population|numerator|denominator|exclusions|exceptions">
            	<xsl:choose>
      ...
      <component typeCode="COMP">
         <section>
            <code code="57025-9" codeSystem="2.16.840.1.113883.6.1"
               displayName="Data criteria (QDS Data Elements)"/>
            <title>Data criteria (QDS Data Elements)</title>
            <text/>
            <xsl:for-each select="elementlookup/qdsel">
               <xsl:if test="(key('NON_DC_IDS',@id) or key('NON_DC_TOS',@id) or key('IQDSEL_IDS',@id)) and .[@oid]">
            ...
            <xsl:for-each select="elementlookup/propel">
           	...
           	<xsl:apply-templates select="elementlookup/measurecalc" mode="dc_measurecalc"/>
           	<xsl:apply-templates select="elementlookup/iqdsel" mode="dc_iqdsel"/>
         </section>
      </component> 
   </xsl:template>
   ...
   <xsl:template match="population|denominator|exclusions|numerator|exceptions">
      <xsl:text>
   ...
   <xsl:template match="*" mode="getId">
	...
	<xsl:template match="*" mode="pauseQuantity">
      ...
      <xsl:choose>
	   <xsl:when test="$unit='a' or $unit= 'm' or $unit= 'd' or $unit= 'h' or $unit= 'w' or $unit= 'min' or $unit= 'qtr'">
	      <pauseQuantity xsi:type="{$vtype}">
	...
	<xsl:template name="unitvalue">
		...
		<xsl:choose>
   	  		...
   	  		<xsl:when test="$uval='month'">m</xsl:when>
			<xsl:when test="$uval='months'">m</xsl:when>
   	  		...
   	  		<xsl:when test="$uval='week'">w</xsl:when>
   	  		<xsl:when test="$uval='weeks'">w</xsl:when>
   ...
   <xsl:template match="*" mode="resultvalue">
      ...
      <xsl:if test="$unit!='a' and $unit!= 'm' and $unit!= 'd' and $unit!= 'h' and $unit!= 'w' and $unit!= 'min' and $unit!= 'qtr'">
	      <xsl:choose>
   ...
   <xsl:template match="headers">
      ...
      <xsl:if test="/measure/headers/availabilityTime/@value">
	      <availabilityTime value="{normalize-space(/measure/headers/availabilityTime/@value)}"/>
	  </xsl:if>
      ...
      <xsl:for-each select="scores/score">
         <subjectOf>
            <measureAttribute>
               ...
               <value xsi:type="CD" code="{@id}" codeSystem="2.16.840.1.113883.5.1063" displayName="{normalize-space(.)}"/>
            </measureAttribute>
         </subjectOf>
      </xsl:for-each>
      ...
      <subjectOf>
         <measureAttribute>
            <code code="MSRAGG" codeSystem="2.16.840.1.113883.5.4" displayName="Data Aggregation"/>
            <value xsi:type="ED" mediaType="text/plain">
				<xsl:call-template name="trim">
   ...
   <xsl:template name="measure_specific_data_elements">
		<subjectOf>
		    <measureAttribute>
		        ...
		        <value xsi:type="EXPR_PQ" value="12" unit="m">
					<expression mediaType="text/plain">measureEndDate - measureStartDate</expression>
				</value>
		    </measureAttribute>
		 </subjectOf>
		<subjectOf>
		...
		<xsl:variable name="duration_unit">
			<xsl:choose>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='a']">year(s)</xsl:when>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='m']">month(s)</xsl:when>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='d']">day(s)</xsl:when>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='min']">minute(s)</xsl:when>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='h']">hour(s)</xsl:when>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='qtr']">quarter(s)</xsl:when>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='w']">week(s)</xsl:when>
				<xsl:when test="/measure/headers/period/duration[lower-case(@unit)='h']">hour(s)</xsl:when>
			</xsl:choose>
		</xsl:variable>
		<subjectOf>
		   <measureAttribute>
		      <code code="MSRMD" codeSystem="2.16.840.1.113883.3.560" displayName="Measurement duration"/>
		      <value xsi:type="ED" mediaType="text/plain">
		      	<xsl:choose>
		      		<xsl:when test="string-length($duration_unit)>0">
						<xsl:call-template name="trim">
	              			 <xsl:with-param name="textString" select="/measure/headers/period/duration"/>
	            		</xsl:call-template>
				      	<xsl:text> </xsl:text>
