      ...
      <xsl:variable name="qdsdisplayname">
		<xsl:choose>
			<xsl:when test="/measure/elementlookup/*[@id=$qdsid]/@taxonomy='GROUPING'">
				<xsl:value-of select="/measure/elementlookup/*[@id=$qdsid]/@name"/><xsl:text> Code List </xsl:text><xsl:value-of select="/measure/elementlookup/*[@id=$qdsid]/@taxonomy"/>
			</xsl:when>
			<xsl:when test="/measure/elementlookup/*[@id=$qdsid]/@taxonomy">
        		<xsl:value-of select="/measure/elementlookup/*[@id=$qdsid]/@name"/><xsl:text> </xsl:text><xsl:value-of select="/measure/elementlookup/*[@id=$qdsid]/@taxonomy"/><xsl:text> Code List</xsl:text>
        	</xsl:when>
        </xsl:choose>
      </xsl:variable>
...
-->
      <xsl:variable name="refdisplayname">
      	<xsl:choose>
	      	<xsl:when test="/measure/elementlookup/*[@id=$refid]/@taxonomy='GROUPING'">
	      		<xsl:value-of select="/measure/elementlookup/*[@id=$refid]/@name"/>
	      		<xsl:text> Code List </xsl:text>
	      		<xsl:value-of select="/measure/elementlookup/*[@id=$refid]/@taxonomy"/>
	      	</xsl:when>
	      	<xsl:when test="/measure/elementlookup/*[@id=$refid]/@taxonomy">
	      		<xsl:value-of select="/measure/elementlookup/*[@id=$refid]/@name"/>
	      		<xsl:text> </xsl:text>
	      		<xsl:value-of select="/measure/elementlookup/*[@id=$refid]/@taxonomy"/><xsl:text> Code List</xsl:text>
	      	</xsl:when>
      	</xsl:choose>      	
      </xsl:variable>
	...
	<xsl:choose>
	    ...
	    <!-- <xsl:when test="contains($refid, 'not done')"> -->
	    <xsl:if test="$refid != 'NA' or $isNot='true'">
		...
		<xsl:comment><xsl:text> </xsl:text><xsl:value-of select="$qdsdatatype"/> pattern </xsl:comment>
		 ...
		 <xsl:choose>
			...
			<xsl:when test="lower-case($qdsdatatype)='patient characteristic'">
				<templateId root="{$tid-root}" /> <!--patient characteristic -->
				<id root="{$qdsuuid}" />
				<code code="ASSERTION" codeSystem="2.16.840.1.113883.5.4"/>
				<sourceOf typeCode="COMP">
					<observation classCode="OBS" moodCode="EVN" isCriterionInd="true">
						<code code="{$qdsoid}"  displayName="{$qdsdisplayname}" />
						<title><xsl:value-of select="$title"/></title>
						<xsl:apply-templates select="properties/property" mode="pre_val"/>
			...
			<!---DEVICE RECOMMENDED-->
			<xsl:when test="lower-case($qdsdatatype)='device recommended' or $refdatatype='device recommended not done' or lower-case($qdsdatatype)='device, recommended' or $refdatatype='device, recommended not done'">
            ...
            <xsl:when test="lower-case($qdsdatatype)='device order' or $refdatatype='device order not done' or lower-case($qdsdatatype)='device, order' or $refdatatype='device, order not done'">
            ...
            <xsl:otherwise>
               <ERROR>
                  ...
                  <DESCRIPTION>No template found matching the data type of
                     "<xsl:value-of select="$qdsdatatype"/>" for the code list
                     "<xsl:value-of select="$qdsname"/>"
                  </DESCRIPTION>
               </ERROR>
            </xsl:otherwise>
	...
	<!-- cannot output <effectiveTime> after <value> -->
	<xsl:template match="property" mode="pre_val">
		<xsl:choose>
			<xsl:when test="@name='discharge' or @name='stop' or @name='stop datetime' or @name='stop datetime id'">
				<effectiveTime xsi:type="IVL_TS">
					<xsl:if test="../property[@name='admission' or @name='admit' or @name='start datetime' or @name='start datetime id']">
						<low/>
					</xsl:if>
					<high />
				</effectiveTime>
			</xsl:when>
			<xsl:when test="@name='admission' or @name='admit' or @name='start datetime' or @name='start datetime id'">
				<xsl:if test="not(../property[@name='discharge' or @name='stop' or @name='stop datetime' or @name='stop datetime id'])">			
					<effectiveTime xsi:type="IVL_TS">
						<low />
					</effectiveTime>
				</xsl:if>
	...
	<xsl:template name="qualifier">
		...
		<xsl:choose>
			<xsl:when test="$qname='NOTEMPTY'">
				<qualifier>
					<name code="405662005" codeSystem="2.16.840.1.113883.6.96" displayName="NOTEMPTY"/>
				</qualifier>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="property" mode="val_qual">
		...
		<xsl:choose>
			<xsl:when test="@name='related to id'">
				<qualifier>
					<name code="404684003" codeSystem="2.16.840.1.113883.6.96" displayName="Care goal"/>
					<value code="{$pcode}"   displayName="{$pdisplayName}"/>
           		</qualifier>
			</xsl:when>
			<xsl:when test="@name='anatomical_structure id'">
				<qualifier>
					<name code="91723000"  codeSystem="2.16.840.1.113883.6.96" displayName="Anatomical Structure"/>
					<value code="{$pcode}"   displayName="{$pdisplayName}"/>
           		</qualifier>
			</xsl:when>
	...
	<xsl:template name="ordinal">
		...
		<sourceOf typeCode="SUBJ">
			<observation classCode="OBS" moodCode="EVN" isCriterionInd="true">
				<code code="117363000" codeSystem="2.16.840.1.113883.6.96" displayName="Ordinal" />
				<value xsi:type="CD" code="{$code}"   displayName="{$displayName}"/>
			</observation>
		</sourceOf>
	</xsl:template>
	...
	<xsl:template match="iqdsel" mode="dc_iqdsel">
		...
		<xsl:variable name="tid-root">
	   	...
	   	<entry typeCode="DRIV" instanceInd="true">
